...
/**
 * 全ての添付ファイルをダウンロード.
 * @see https://github.com/apache/commons-net/blob/master/src/main/java/org/apache/commons/net/examples/mail/IMAPExportMbox.java
 *
 * @author cyrus
 */
public class IMAPExportMbox {
				...
				if (m.lookingAt()) { // found a match
					...
					if (lastSeq != -1) {
						...
						if (missing != 0) {
							...
							System.err.println("*** Sequence error: current=" + msgSeq + " previous=" + lastSeq
									+ " Missing=" + missing);
						}
					}
					lastSeq = msgSeq;
	...
	public static void main(final String[] args) throws IOException, URISyntaxException {
		...
		if (argCount < 2) {
			...
			System.err
					.println("\tPrefix file name with '+' to append to the file. Prefix with '-' to allow overwrite.");
		...
		if (file.equals("-")) {
			...
			mboxListener = new MboxListener(new BufferedWriter(new FileWriter(mbox, true)), eol, printHash, printMarker,
					checkSequence);
		} else if (file.startsWith("-")) {
			final File mbox = new File(file.substring(1));
			System.out.println("Writing to file " + mbox);
			mboxListener = new MboxListener(new BufferedWriter(new FileWriter(mbox, false)), eol, printHash,
